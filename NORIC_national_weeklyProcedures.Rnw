...
if (is.element(format(Sys.time(), "%a"), dat$DAG)) {
todaysBrew <- rev(weeklyBrew[1:numberOfDays])
} else {
todaysBrew <- rev(weeklyBrew[2:(numberOfDays+1)])
}
dat$DAG <- recode(dat$DAG, "'Sun'='Søn';'Mon'='Man';'Tue'='Tir';'Wed'='Ons';'Thu'='Tor';'Fri'='Fre';'Sat'='Lør'")
#dat$DAG <- factor(dat$DAG, levels = c("Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"))
dat$DAG <- factor(dat$DAG, levels = c("Søn", "Man", "Tir", "Ons", "Tor", "Fre", "Lør"))
...
p1 <- ggplot(dat, aes(x=SHUS, fill=DAG)) + geom_bar() + scale_fill_manual(values=todaysBrew) + labs(y = "Antall", title=dateTimeStamp) + guides(fill = guide_legend(reverse = TRUE)) + coord_flip() + scale_x_discrete(drop=FALSE) + theme(axis.text.y=element_text(colour="black", size=12)) + theme(axis.title.y = element_blank())
@
