...
# to be used in case of annotations. assume latex source, always
pdfAttachFiles <- 'NORIC_national_annual_report.tex'
...
pdfAttachFiles <- c(pdfAttachFiles, file)
pdf(file , width = 12 , height = 6 , pointsize = 12)
...
% removed for now since I do not have the png-file
\iffalse
\section{Arbeitsflyt}
\begin{figure}
  \label{fig:workflow}
...
% make backmatter available (since document is class report)
\makeatletter
...
<<addAnnotation, echo=F, results=tex, eval=T>>=
...
if (usePdfAnnotation) {
  ...
  pdfAttachFiles <- character(0)
}
@

\SweaveInput{SweaveChildAddAnnotation.Rnw}
