...
\begin{figure}
  \centering
  \caption{Prosent andel prosedyrer etter hastegrad og PCI senter}
<<echo=FALSE , results = tex>>=
    ...
    xtable(
        100 * prop.table(
            x = xtabs(
                ...
                data = dat) ,
            margin = 1) ,
        digits = 1 , caption = "Prosent andel prosdyrer etter hastegrad og PCI senter") ,
    booktabs = TRUE ,
    rotate.colnames = TRUE)
@
\end{tiny}
...
\begin{figure}
  \centering
  \caption{Prosent andel prosedyrer etter type og PCI senter}
<<echo=FALSE , results = tex>>=
    ...
    xtable(
        100 * prop.table(
            x = xtabs(
                ...
                data = dat) ,
            margin = 1) ,
        digits = 1 , caption = "Prosent andel prosdyrer etter type og PCI senter") ,
    booktabs = TRUE ,
    rotate.colnames = TRUE)
@
\end{tiny}
...
\begin{figure}
  \centering
  \caption{Prosent andel prosedyrer etter primær beslutning og PCI senter}
<<echo=FALSE , results = tex>>=
...
\begin{figure}
  ...
  \caption{Antall prosedyrer etter primær beslutning og PCI senter}
<<echo=FALSE , results = tex>>=
file = "N_procedures_primbes_detailed_PCIcenter.pdf"
pdf(file , width = 12 , height = 8 , pointsize = 12)
...
barplot(
    xtabs(~ PrimaerBeslutning2 + factor(SHUS)  , data = dat) , 
    las = 1 , 
    xlab = "PCI Senter" , 
    ylab = "Antall" , 
    col = pal , 
...
legend(
    ...
    horiz = FALSE , 
    fill = pal, 
    legend = levels(dat$PrimaerBeslutning2) , 
    cex = 1)
par(op)
...
print(
    xtable(
        addmargins( xtabs( ~ SHUS + PrimaerBeslutning2 , dat)) ,
        digits = 0 ,
        caption = "Antall prosedyrer etter primær beslutning og PCI senter") ,
    booktabs = TRUE ,
    rotate.colnames = TRUE)
...
\begin{figure}
  ...
  \caption{Prosent andel prosedyrer etter primær beslutning og PCI senter}
<<echo=FALSE , results = tex>>=
file = "r_procedures_primbes_detailed_PCIcenter.pdf"
pdf(file , width = 12 , height = 8 , pointsize = 12)
...
print(
    xtable(
        100 * prop.table(
            x = addmargins(
                A = xtabs( ~ SHUS + PrimaerBeslutning2 , dat) ,
                margin = 1) ,
            margin = 1) ,
        digits = 1 ,
        caption = "Proent andel prosedyrer etter primær beslutning og PCI senter") ,
    booktabs = TRUE ,
    rotate.colnames = TRUE)
@
\end{tiny}
...
\begin{figure}
  \centering
  \caption{Prosent andel prosdyrer etter vakttid og PCI senter}
<<echo=FALSE , results = tex>>=
    ...
    xtable(
        100 * prop.table(
            x = addmargins(
                ...
                margin = 1 ) ,
            margin = 1 ) ,
        digits = 1 , caption = "Prosent andel prosedyrer etter vakttid og PCI senter") ,
    booktabs = TRUE ,
    rotate.colnames = TRUE)
@
\end{tiny}
...
\begin{figure}
  \centering
  \caption{Prosent andel prosedyrer etter indikasjon og PCI senter}
<<echo=FALSE , results = tex>>=
...
\begin{figure}
  \centering
  \caption{Prosent andel prosedyrer etter funn og PCI senter}
<<echo=FALSE , results = tex>>=
...
\begin{tiny}
<<echo = FALSE , results = tex>>=
FFR <- xtabs( ~ SHUS + DIATRYCK , data = dat )[ , 2]
FFR <- data.frame( SHUS = attr( x = FFR , which = "names") , FFR = FFR )

## ingen Doppler registrert
#Doppler <- xtabs( ~ SHUS + DIADOP , data = dat )[ , 2]
#Doppler <- data.frame( SHUS = attr( x = Doppler , which = "names") , Doppler = Doppler )

IVUS <- xtabs( ~ SHUS + DIAIVUS , data = dat )[ , 2]
IVUS <- data.frame( SHUS = attr( x = IVUS , which = "names") , IVUS = IVUS )
...
print(
    xtable(
        Tillegg ,
        digits = 0 , caption = "Antall tilleggsundersøkelser etter PCI senter") ,
    booktabs = TRUE ,
...
For 2013 er det ved HUS registrert 3035 angiografier og 1290 PCI i NORIC og 3133 angiografier og 1360 PCIer  i Orbit. Dette gir en dekningsgrad på henholdsvis 97 og 95 \%.
...
Under utfyllingen av registreringsskjema blir variabler der dette er mulig og aktuelt, sjekket mot andre variabler for å sikre samsvar mellom disse. Kvaliteten på data blir også forsøkt sikret ved å sjekke at de verdiene som blir lagt inn, er innenfor grenser som er sannsynlige eller tillatt.
...
Pr. september 2014 er 6 av totalt 8 invasive sentre i gang med registrering av data i NORIC. Vi vil arbeide for at de to resterende sykehusene også skal ta i bruk kvalitetsregisteret. Dekningsgradsanalyser vil bli gjennomført ved hjelp av data fra NPR når vi er koblet opp mot det Nasjonale hjerte- og karregisteret. For det enkelte sykehus vil vi tilstrebe løsninger for fortløpende kontroll (daglig/ukentlig) mot andre kliniske registreringssystem for å sjekke at alle pasienter blir registrert. Dette er allerede implementert ved Haukeland universitetssjukehus ved kontroll mot Orbit som blir brukt til planlegging av programmet på laboratoriene. 
...
Våre muligheter til å registrere demografiske og sosiale variabler er begrenset siden disse ikke er hjemlet i forskriften for Hjerte- og karregisteret. 
